...
- 住客通过私密管理链接查看状态或取消申请
- Admin 审计日志、冲突预览，以及支持安全恢复和确认后永久删除的回收站
- 可撤销的私密日历订阅、脱敏 CSV/JSON 导出和仅 Admin 可访问的健康检查
- 按完整日期范围检查容量并自动分配睡位
...
Admin 删除住宿记录时，记录会先进入回收站并立即释放床位。恢复已确认记录时，
系统会重新检查不可住时段、独占冲突和容量。回收站记录也可在明确二次确认后
永久删除；该操作不可撤销，并会级联清理关联的床位分配与修改请求。
