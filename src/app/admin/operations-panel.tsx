...
import { resetCalendarFeed, restoreRequest } from "@/app/actions";
...
export default function OperationsPanel({logs,trash,feedToken}:{logs:AuditLog[];trash:BookingRequest[];feedToken:string}){
  ...
  return <AdminPanel eyebrow="OPERATIONS" title={t("审计、导出与回收站")} description={t("私密工具只对已登录 Host 开放。") }>
    ...
    <details><summary>{t("回收站")} · {trash.length}</summary>
      {trash.map(r=><form action={restoreRequest} key={r.id} className="danger-zone"><input type="hidden" name="id" value={r.id}/><span><b>{r.guest_name}</b><small>{r.stay_name} · {r.starts_on} → {r.ends_on}</small></span><SubmitButton pendingLabel={t("处理中…")}>{t("恢复并重新检查床位")}</SubmitButton></form>)}
